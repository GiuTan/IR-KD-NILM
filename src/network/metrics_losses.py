...
# def loss_dist(y_true, y_pred,confidences,conf_use):
#     y_true = tf.cast(y_true, tf.float32)
#
#     new_true = tf.multiply(y_true, tf.cast(tf.not_equal(y_true, -1), tf.float32))
#     new_pred = tf.multiply(y_pred, tf.cast(tf.not_equal(y_true, -1), tf.float32))
#     if conf_use:
#         loss = K.binary_crossentropy(new_true, new_pred) * tf.squeeze(confidences,axis=2) #
#     else:
#         loss = K.binary_crossentropy(new_true, new_pred)
#
#     return tf.reduce_mean(loss)
...
class GradNormRegularizedLoss(Loss):
    def __init__(self, reg_constant=1e-3):
        super().__init__()
        self.reg_constant = reg_constant

    def call(self, y_true, y_pred):
        raw_loss = tf.keras.losses.categorical_crossentropy(y_true, y_pred)
        with tf.GradientTape() as tape:
            tape.watch(y_pred)
            loss = tf.keras.losses.categorical_crossentropy(y_true, y_pred)
        grads = tape.gradient(loss, y_pred)
        grad_norm = 1e5 * tf.reduce_sum(tf.square(grads)) / tf.cast(tf.shape(y_pred), tf.float32)
        return raw_loss + self.reg_constant * grad_norm
...
def xai_cosine_loss(teacher_explanation, student_explanation, axis=1, confidences=None, conf_used=False):
    ...
    if conf_used:
        cosine_similarity_loss = tf.multiply(cosine_similarity_loss,tf.squeeze(confidences,axis=2))  # None,1
